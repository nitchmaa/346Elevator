...
void loop() {
  while(1){
    ...
    stepTop.moveTo(3*SPR/8); //Set the target motor position (i.e. turn motor for 3/8 full revolutions)
    stepTop.runToPosition(); // Run the motor to the target position
    delay(500);
    stepTop.moveTo(-3*SPR/8); //Set the target motor position (back to start)
    stepTop.runToPosition(); // Run the motor to the target position (start)
    delay(1000);
    Serial.println("Testing: Bottom Stepper Motor");
    stepBot.moveTo(3*SPR/8); //Set the target motor position (i.e. turn motor for 3/8 full revolutions)
    stepBot.runToPosition(); // Run the motor to the target position
    delay(500);
    stepBot.moveTo(-3*SPR/8); //Set the target motor position (back to start)
